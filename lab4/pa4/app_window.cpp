...
void AppWindow::glutKeyboard ( unsigned char key, int x, int y )
 {
   switch ( key )
    { case ' ': _viewaxis != _viewaxis; redraw(); break;
	  ...
	  case 'c':normson = 1; redraw();break;
	  case'v':normson = 0; redraw();break;
	  case 'q': nfaces++; redraw(); break;
	  case 'a': if(nfaces > 1) nfaces--; redraw(); break;
	  case 'x':normtype = 0; redraw(); break;
	  case 'z':normtype = 1; redraw(); break;
	  case 'w':l.pos.x = l.pos.x + .01;redraw() ; break;
	  case 's': l.pos.x = l.pos.x - .01; redraw(); break;
...
void AppWindow::glutDisplay ()
 {
   ...
   if ( _axis.changed ) // needs update
    { _axis.build(1.0); // axis has radius 1.0
    }

   if (_cap.changed) // needs update
   {
	   _cap.build(nfaces,normtype);
   }

   if (no.changed) // needs update
   {
	   no.build(nfaces, normtype);
   }
   ...
   if (_viewaxis) _cap.draw(stransf, sproj,l,m);
   if (normson == 1) no.draw(stransf, sproj);
